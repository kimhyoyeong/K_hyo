...
ImageSlider.prototype.startAutoPlay=function(){
    if(this.timerID==0){//타이머 아이디
        this.timerID= setInterval($.proxy(function(){//(setInterval일정시간마다 반복)($.proxy란 this 함수를 내부에서도 사용하기 위해)
            this.nextImage();
...
ImageSlider.prototype.stopAutoPlay=function(){
    if(this.timerID!=0){
        clearInterval(this.timerID);//setInterval 생성된 변수 this.timerID가 반복하고 있는걸 스탑
        this.timerID=0;
    }
}
